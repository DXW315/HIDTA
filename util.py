...
# PathChoiceDecoder 和 SequentialConsistencyLoss 模块已经引入
...
def load_hadj(pkl_filename, top_k):
    ...
    H_all = np.zeros((l, n))
    edge_1 = np.array([])  # 全部为 1 的矩阵
    edge_2 = np.array([])  # 也是全部为 1 的矩阵
    a = 0
    ...
    invDE_all = np.asmatrix(np.diag(DE__all))
    DV2_all = np.asmatrix(np.diag(np.power(DV_all, -0.5)))
    W_all = np.asmatrix(np.diag(W_all))
    H_all = np.asmatrix(H_all)
    ...
    return H_a, H_b, H_T_new, lwjl, G0, G1, indices, G0_all, G1_all
